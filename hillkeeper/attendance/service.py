...
async def send_morning_check(bot, channel_id: str, role_id: str):
    ...
    try:
        ...
        # Redis에 저장
        await repository.save_message(message.id, channel_id=channel.id)
...
async def send_evening_reminder(bot, channel_id: str, role_id: str):
    ...
    try:
        ...
        # Redis에서 메시지 ID 가져오기
        message_ids = await repository.get_all_messages()
        ...
        for message_id in message_ids:
            try:
                ...
                # 실패한 메시지는 Redis에서 삭제
                await repository.delete_message(message_id)
        ...
        # Redis 정리
        await repository.clear_all_messages()
